"""Tests for the pyarpscan module."""
...
class TestListInterfaces:
    ...
    def test_returns_list(self):
        ...
        result = pyarpscan.list_interfaces()
...
class TestHostClass:
    ...
    def test_host_attributes_exist(self):
        ...
        assert hasattr(pyarpscan.Host, "ip")
        assert hasattr(pyarpscan.Host, "mac")
        assert hasattr(pyarpscan.Host, "vendor")
...
class TestScanNetworkErrors:
    ...
    def test_invalid_interface_raises_value_error(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            pyarpscan.scan_network("nonexistent_interface_xyz123")
    ...
    def test_invalid_cidr_raises_error(self):
        ...
        with pytest.raises((ValueError, PermissionError)):
            pyarpscan.scan_network("lo", network="invalid_cidr")
    ...
    def test_permission_error_without_cap_net_raw(self):
        ...
        try:
            result = pyarpscan.scan_network("lo", network="127.0.0.0/8", timeout=1)
...
class TestScanNetworkSignature:
    ...
    def test_timeout_parameter_accepts_integer(self):
        ...
        with pytest.raises(ValueError):
            pyarpscan.scan_network("nonexistent_xyz", timeout=5)
    ...
    def test_network_parameter_is_optional(self):
        ...
        with pytest.raises(ValueError):
            pyarpscan.scan_network("nonexistent_xyz")
...
class TestModuleAttributes:
    ...
    def test_module_has_scan_network(self):
        ...
        assert hasattr(pyarpscan, "scan_network")
        assert callable(pyarpscan.scan_network)

    def test_module_has_list_interfaces(self):
        """Module should export list_interfaces function."""
        assert hasattr(pyarpscan, "list_interfaces")
        assert callable(pyarpscan.list_interfaces)

    def test_module_has_host_class(self):
        """Module should export Host class."""
        assert hasattr(pyarpscan, "Host")
